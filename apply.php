<?php
   ...
   if( $_GET["title"]  ) {
      ...
      $email=$_SESSION['user'];
      $check_if_resume="select * from resume WHERE email='$email'";
      $run_query=$dbcon->query($check_if_resume);  
      ...
      $check_if_paid="select * from resume WHERE email='$email'";
      $run_query=$dbcon->query($check_if_paid);  
      ...
      if($run_query->num_rows==0){
       ...
       $check_if_exists="select * from apply WHERE user='$email' AND job='$id'";
        $run_query=$dbcon->query($check_if_exists);  
